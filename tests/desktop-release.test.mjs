...
test("本地 assembly 生成 darwin updater manifest 并保留可下载的稳定文件名", async () => {
  ...
  try {
    ...
    assert.equal(
      updater.url,
      "http://127.0.0.1:41731/downloads/turbo/ai-cove-turbo-macos-aarch64.app.tar.gz?v=0.1.0-pre.1",
    );
...
test("CI assembly 从双平台 updater 实物生成合并 manifest", async () => {
  ...
  try {
    ...
    await writeFile(path.join(darwinDir, "AI Cove Turbo_0.1.0-pre.1_aarch64.dmg"), "dmg");
    ...
    await writeFile(path.join(windowsDir, "AI Cove Turbo_0.1.0-pre.1_x64-setup.exe"), "exe");
    await writeFile(
      path.join(windowsDir, "AI Cove Turbo_0.1.0-pre.1_x64-setup.exe.sig"),
      "windows signature\n",
    );
    ...
    assert.equal(
      ...
      "https://ai-cove.com/downloads/turbo/ai-cove-turbo-macos-aarch64.app.tar.gz?v=0.1.0-pre.1",
    );
    assert.equal(manifest.platforms["windows-x86_64"].signature, "windows signature");
    assert.equal(
      manifest.platforms["windows-x86_64"].url,
      "https://ai-cove.com/downloads/turbo/ai-cove-turbo-windows.exe?v=0.1.0-pre.1",
    );
