...
**Codex 配置位置**：Turbo 只读取当前用户目录下的默认 `.codex/config.toml`；文件不存在、无法解析或缺少受管 Provider 时停止接管，并提示用户重试。公共 AI Cove 子域名若在配置中写成 `http://`，Turbo 会明确提示改为 HTTPS，不会把它显示成未知或离线故障。
_Avoid_: 读取备份文件、读取 `CODEX_HOME` 作为文件名、自动创建或修复 Provider。
